...
regenerate_sot() {
    echo "Regenerating sot.json (full pipeline: index -> map, rust indexer)..." >&2

    # Build Rust indexer if not present
    local rust_bin="$REPO_ROOT/kloc-indexer-php/target/release/kloc-indexer-php"
    if [[ ! -x "$rust_bin" ]]; then
        echo "Building Rust indexer..." >&2
        (cd "$REPO_ROOT/kloc-indexer-php" && cargo build --release) >&2
    fi

    # Read internal_all flag from cases.json
    local internal_flag=""
    local has_internal
    has_internal=$(python3 -c "import json; d=json.load(open('$CASES_FILE')); print('1' if d.get('internal_all') else '')")
    if [[ -n "$has_internal" ]]; then
        internal_flag="--internal-all"
    fi

    rm -f "$ARTIFACT_DIR/index.json" "$ARTIFACT_DIR/sot.json"
    "$REPO_ROOT/kloc-dev.sh" --id="$SOT_ID" --rust-indexer $internal_flag >&2
    echo "" >&2
